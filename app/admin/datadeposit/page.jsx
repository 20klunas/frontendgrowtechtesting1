...
function UserSearchSelect({ label, value, search, onSearchChange, users, onChange, placeholder = 'Cari email user...' }) {
  return (
...
export default function DataDepositPage() {
  ...
  const selectedManualUser = useMemo(() => users.find((user) => String(user.id) === String(manualTopup.user_id)) || null, [users, manualTopup.user_id])
  const selectedAdjustUser = useMemo(() => users.find((user) => String(user.id) === String(adjustForm.user_id)) || null, [users, adjustForm.user_id])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => fetchUsers(userSearch), 250)
    return () => clearTimeout(timer)
  }, [userSearch, activeTab])

  useEffect(() => {
    if (activeTab !== 'adjust') return
    const timer = setTimeout(() => fetchUsers(adjustUserSearch), 250)
    return () => clearTimeout(timer)
  }, [adjustUserSearch, activeTab])
  ...
  const fetchTopups = async (page = 1) => {
    try {
      ...
      const res = await fetch(`${API}/api/v1/admin/wallet/topups?${params.toString()}`, { headers: { Accept: 'application/json', Authorization: `Bearer ${token}` } })
  ...
  const fetchLedger = async (page = 1) => {
    try {
      setLoading(true)
      const res = await fetch(`${API}/api/v1/admin/wallet/ledger?per_page=10&page=${page}`, { headers: { Accept: 'application/json', Authorization: `Bearer ${token}` } })
      const result = await res.json()
      if (result.success) {
  ...
  const fetchUsers = async (search = '') => {
    try {
      ...
      const res = await fetch(`${API}/api/v1/admin/users?${params.toString()}`, { headers: { Accept: 'application/json', Authorization: `Bearer ${token}` } })
      const result = await res.json()
  ...
  const handleManualTopup = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/api/v1/admin/wallet/topup`, {
        ...
        body: JSON.stringify({ user_id: Number(manualTopup.user_id), amount: Number(manualTopup.amount), note: buildNote(manualTopup.reason, manualTopup.reference, manualTopup.detail) })
      })
      const result = await res.json()
      if (result.success) {
    ...
    <PermissionGate permission="manage_wallets">
      <section className="p-6">
        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          <button onClick={() => setActiveTab('topups')} className={activeTab === 'topups' ? 'btn-primary' : 'btn-outline'}>User Deposit</button>
          <button onClick={() => setActiveTab('ledger')} className={activeTab === 'ledger' ? 'btn-primary' : 'btn-outline'}>Ledger</button>
          <button onClick={() => setActiveTab('manual')} className={activeTab === 'manual' ? 'btn-primary' : 'btn-outline'}>Manual Topup</button>
          <button onClick={() => setActiveTab('adjust')} className={activeTab === 'adjust' ? 'btn-primary' : 'btn-outline'}>Adjust Balance</button>
        </div>
        ...
        {activeTab === 'topups' && (
          <div className="border rounded-xl p-4">
            <div className="flex gap-3 mb-4 flex-wrap">
              ...
              <input className="input" placeholder="User ID" value={userIdFilter} onChange={(e) => setUserIdFilter(e.target.value)} />
              <button onClick={() => fetchTopups()} className="btn-outline">Filter</button>
            </div>

            <table className="w-full text-left">
              <thead><tr className="border-b"><th>ID</th><th>Order</th><th>Status</th><th>User</th><th>Amount</th><th>Tanggal</th></tr></thead>
              <tbody>
                {topups.map((item) => (
                  <tr key={item.id} className="border-b text-white">
                    <td>{item.id}</td><td>{item.order_id}</td><td>{item.status}</td><td>{item.user?.email || item.user?.name || '-'}</td><td>Rp {formatRupiah(item.amount)}</td><td>{formatDateTime(item.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === 'ledger' && (
          <div className="border rounded-xl p-4 overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead><tr className="border-b"><th>ID</th><th>Wallet</th><th>Email</th><th>Direction</th><th>Amount</th><th>Before</th><th>After</th><th>TX Type</th><th>Status</th><th>Note</th><th>Created</th></tr></thead>
              <tbody>
                {ledger.map((item) => (
                  <tr key={item.id} className="border-b text-white">
                    <td>{item.id}</td>
                    <td>{item.wallet?.code ?? `Wallet #${item.wallet_id}`}</td>
                    <td>{item.wallet?.user?.email ?? '-'}</td>
                    <td><span className={item.direction === 'CREDIT' ? 'text-green-500 font-semibold' : 'text-red-500 font-semibold'}>{item.direction}</span></td>
            ...
            {ledgerPagination && (
              <div className="flex justify-end gap-3 mt-4">
                <button disabled={!ledgerPagination.prev_page_url} onClick={() => fetchLedger(ledgerPagination.current_page - 1)} className="btn-outline">Prev</button>
                <span>Page {ledgerPagination.current_page} / {ledgerPagination.last_page}</span>
                <button disabled={!ledgerPagination.next_page_url} onClick={() => fetchLedger(ledgerPagination.current_page + 1)} className="btn-outline">Next</button>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'adjust' && (
          <form onSubmit={handleAdjust} className="border rounded-xl p-4 space-y-4">
            <UserSearchSelect label="Email User" value={adjustForm.user_id} search={adjustUserSearch} onSearchChange={setAdjustUserSearch} users={users} onChange={(value) => setAdjustForm({ ...adjustForm, user_id: value })} />
            {selectedAdjustUser ? <div className="text-sm text-gray-300">Target adjust: <span className="font-semibold text-white">{selectedAdjustUser.email}</span></div> : null}
            <select className="input w-full" value={adjustForm.direction} onChange={(e) => setAdjustForm({ ...adjustForm, direction: e.target.value })}><option value="credit">Credit</option><option value="debit">Debit</option></select>
            <input className="input w-full" type="number" placeholder="Amount" value={adjustForm.amount} onChange={(e) => setAdjustForm({ ...adjustForm, amount: e.target.value })} required />
            <textarea className="input w-full" placeholder="Note" value={adjustForm.note} onChange={(e) => setAdjustForm({ ...adjustForm, note: e.target.value })} />
