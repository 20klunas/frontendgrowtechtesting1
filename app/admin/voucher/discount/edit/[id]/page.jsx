...
export default function EditDiscountPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/v1/admin/discount-campaigns/${id}`, {
      ...
      .then((json) => {
        ...
        setForm({
          name: data.name || data.nama_discount || '',
          ...
          stack_policy: data.stack_policy || 'stackable',
          targets: data.targets || [],
          tier_rules: {
            allowed_tiers: Array.isArray(data.tier_rules?.allowed_tiers) ? data.tier_rules.allowed_tiers : [],
            excluded_tiers: Array.isArray(data.tier_rules?.excluded_tiers) ? data.tier_rules.excluded_tiers : [],
          },
        })
      })
  ...
  const hasConflict = useMemo(() => {
    ...
    return existingDiscounts.some((discount) => {
      if (discount.id === Number(id)) return false
  ...
  const addTarget = (type, targetId) => {
    const safeId = Number(targetId)
    if (!safeId) return

    setForm((prev) => {
      if (prev.targets.some((target) => target.type === type && Number(target.id) === safeId)) return prev
      return { ...prev, targets: [...prev.targets, { type, id: safeId }] }
    })
  }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(json?.error?.message || 'Gagal update discount')
      }

      router.push('/admin/voucher/discount')
    } catch (error) {
      console.error(error)
      alert(error?.message || 'Gagal update discount')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-10 max-w-6xl mx-auto text-white space-y-6">
      ...
      <Section title="Target Discount">
        <SearchableDropdown label="Target Subcategory" placeholder="Cari subcategory..." search={subcategorySearch} setSearch={setSubcategorySearch} items={filteredSubcategories} onSelect={(targetId) => addTarget('subcategory', targetId)} />
        <SearchableDropdown label="Target Product" placeholder="Cari product..." search={productSearch} setSearch={setProductSearch} items={filteredProducts} onSelect={(targetId) => addTarget('product', targetId)} />
      ...
      <div className="flex justify-end gap-3">
        ...
        <button onClick={handleUpdate} disabled={saving} className="rounded-xl bg-purple-600 px-5 py-3 font-semibold hover:bg-purple-700 disabled:opacity-50">{saving ? 'Menyimpan...' : 'Update Discount'}</button>
      </div>
    </div>
  )
}
